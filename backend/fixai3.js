...
// We are going to replace lines 151 to 167 (0-indexed, inclusive) with the new block.
// Let's first verify what those lines are in the backup.
// We'll print them to console for debugging, but for now we assume we know the range.
// We'll replace 17 lines (151 to 167 inclusive) with 7 lines.
...
const newBlock = [
    ...
    '            contents.push({ role: \\'function\\' as const, parts: [{ functionResponse: { name: call.name, response: formattedResults } }] });',
...
// Remove the old 17 lines (from index 151 to 167 inclusive) and insert the new block.
lines.splice(151, 17, ...newBlock);
